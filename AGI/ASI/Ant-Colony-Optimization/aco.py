...
class Graph:
    def __init__(self, distances):
        ...
        self.pheromones = np.ones_like(
            distances, dtype=float
        )  # Start with equal pheromones
...
class ACO:
    def __init__(self, graph, num_ants, num_iterations, decay=0.5, alpha=1.0):
        ...
        self.best_distance_history = (
            []
        )  # Store the best distance found in each iteration
    ...
    def run(self):
        ...
        for _ in range(self.num_iterations):
            ants = [
                Ant(self.graph) for _ in range(self.num_ants)
            ]  # Create a group of ants
            for ant in ants:
            ...
            self.best_distance_history.append(
                best_distance
            )  # Save the best distance for each iteration
        return best_path, best_distance

    # Update the pheromones on the paths after all ants have completed their trips
    def update_pheromones(self, ants):
        self.graph.pheromones *= (
            self.decay
        )  # Reduce pheromones on all paths (evaporation)
        ...
        for ant in ants:
            for i in range(len(ant.path) - 1):
                ...
                self.graph.pheromones[from_node][to_node] += (
                    self.alpha / ant.total_distance
                )
...
distances = np.random.randint(
    1, 100, size=(num_nodes, num_nodes)
)  # Random distances between 1 and 100
...
aco = ACO(
    graph, num_ants=10, num_iterations=30
)  # Initialize ACO with 10 ants and 30 iterations
