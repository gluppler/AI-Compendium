...
def objective_function(weights, returns, covariance):
    ...
    )  # We want to maximize return and minimize risk
...
def update_particles(particles, global_best_position, returns, covariance, w, c1, c2):
    ...
    for particle in particles:
        ...
        r1, r2 = np.random.rand(len(particle.position)), np.random.rand(
...
def pso_portfolio_optimization(n_particles, n_iterations, returns, covariance):
    ...
    for _ in range(n_iterations):
        update_particles(
            particles, global_best_position, returns, covariance, w, c1, c2
        )
        for particle in particles:
            score = objective_function(particle.position, returns, covariance)
...
covariance = np.array(
    [
        [0.1, 0.02, 0.03],  # Covariance matrix for asset risks
        [0.02, 0.08, 0.04],
        [0.03, 0.04, 0.07],
    ]
)
...
best_weights, optimization_history = pso_portfolio_optimization(
    n_particles, n_iterations, returns, covariance
)
